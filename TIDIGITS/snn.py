...
for isolated spoken digits classification task.
...
def acc_vs_ft_maps(N=10, init_seed=0, nb_winners=None):
    output = {}
    for n_ft_maps in [5,10,20,30,40,50,60,70,90,110,130,150]:
        snn_params = DEFAULT_NETWORK_PARAMS.copy()
        snn_params["conv_out_channels"] = n_ft_maps
        if nb_winners is not None:
            snn_params["conv_nb_winners"] = nb_winners
        recorded_acc = np.zeros(N)
        for i,seed in enumerate(range(init_seed,init_seed+N)):
            recorded_acc[i] = main(seed=seed, snn_params=snn_params, extended_print=False)
        print("\n\n")
        print("##############################################")
        print(f"Mean for {n_ft_maps} out channels : {recorded_acc.mean()} +- {recorded_acc.std()}")
        print(recorded_acc)
        output[n_ft_maps] = recorded_acc
        with open(f'logs/acc_vs_ft_maps{f"_{nb_winners}_winners" if nb_winners is not None else ""}.pkl', 'wb') as f:
...
def acc_vs_pooling(N=10, init_seed=0):
    output = {}
    for pool_size in [1,2,3,4,5,6]:
        snn_params = DEFAULT_NETWORK_PARAMS.copy()
        snn_params["pool_kernel_size"] = pool_size
        snn_params["pool_stride"] = pool_size
        recorded_acc = np.zeros(N)
        for i,seed in enumerate(range(init_seed,init_seed+N)):
            recorded_acc[i] = main(seed=seed, snn_params=snn_params, extended_print=False)
        print("\n\n")
        print("##############################################")
        print(f"Mean for {pool_size} pooling size : {recorded_acc.mean()} +- {recorded_acc.std()}")
        print(recorded_acc)
        output[pool_size] = recorded_acc
        with open('logs/acc_vs_pooling.pkl', 'wb') as f:
            pickle.dump(output, f)
        print("##############################################")
        print("\n\n")
    print("\n\n\n")
...
def acc_vs_lr(N=10, init_seed=0, lr=0.01, adaptive=True, save_weights=False):
    ...
    for max_train_samples in [0,10,50,100,150,200,250,300,500,1000,2000]:
        ...
        for i,seed in enumerate(range(init_seed,init_seed+N)):
            save_w = False
            if save_weights and i == N-1:
                save_w = True
            recorded_acc[i] = main(seed=seed, snn_params=snn_params, max_train_samples=max_train_samples, save_weights=save_w, extended_print=False)
        print("\n\n")
        print("##############################################")
        print(f"Mean for {lr}-{max_train_samples} : {recorded_acc.mean()} +- {recorded_acc.std()}")
        print(recorded_acc)
        output[max_train_samples] = recorded_acc
        with open(f'logs/acc_vs_lr_{lr}{"_adaptive" if adaptive else ""}.pkl', 'wb') as f:
            pickle.dump(output, f)
        print("##############################################")
        print("\n\n")
    print("\n\n\n")
...
def convergence_vs_vdsp_factor(N=10, init_seed=0):
    ...
    for vdsp_dep_factor in [1+0.25*i for i in range(9)]:
        snn_params = DEFAULT_NETWORK_PARAMS.copy()
        snn_params["conv_vdsp_dep_factor"] = vdsp_dep_factor
        recorded_out = np.zeros(N)
        for i,seed in enumerate(range(init_seed,init_seed+N)):
            recorded_out[i] = main(seed=seed, return_nb_train_samples=True, snn_params=snn_params, extended_print=False)
        output[vdsp_dep_factor] = recorded_out
        with open('logs/convergence_vs_vdsp_factor.pkl', 'wb') as f:
            pickle.dump(output, f)
    print(output) 
...
def acc_vs_vdsp_factor(N=10, init_seed=0):
    ...
    for vdsp_dep_factor in [1+0.25*i for i in range(9)]:
        ...
        for i,seed in enumerate(range(init_seed,init_seed+N)):
            recorded_acc[i] = main(seed=seed, snn_params=snn_params, extended_print=False)
        output[vdsp_dep_factor] = recorded_acc
        with open('logs/acc_vs_vdsp_factor.pkl', 'wb') as f:
            pickle.dump(output, f)
    print(output) 
